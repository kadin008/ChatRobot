...
class SequenceToSequence(object):
    ...
    def __init__(self,
        ...
        if share_embedding:
            assert input_vocab_size == target_vocab_size, '如果打开 share_embedding，两个vocab_size必须一样'
        ...
        assert attention_type.lower() in (
            'bahdanau', 'luong'), 'attention_type 必须是 "bahdanau" 或 "luong" 而不是 "{}" '.format(attention_type)

        assert beam_width < target_vocab_size, 'beam_width {} 应该小于 target vocab size {}'.format(
        ...
        assert self.optimizer.lower() in (
            'adadelta', 'adam', 'rmsprop', 'momentum',
            'sgd'), 'optimizer 必须是下列之一： adadelta, adam, rmsprop, momentum, sgd'
