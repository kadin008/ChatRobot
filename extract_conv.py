...
def main(limit=20, x_limit=3, y_limit=6):
    ...
    for group in tqdm(groups):
        for i, line in enumerate(group):
            ...
            if next_lien:
                x_data.append(line)
                y_data.append(next_lien)
            if last_line and next_lien:
                x_data.append(last_line + make_split(last_line) + line)
                y_data.append(next_lien)
            if next_lien and next_next_line:
                x_data.append(line)
                y_data.append(next_lien + make_split(next_lien) + next_next_line)
